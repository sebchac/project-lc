...
jurisprudencia, dónde hay contradicciones entre niveles de autoridad y qué temas no
tienen masa doctrinal propia. El \textbf{sistema RAG vectorial} indexa el texto
completo y genera respuestas con cita literal al
considerando exacto.

El \textit{pipeline} es replicable, de código abierto y neutro respecto del tamaño del
\textit{corpus}. El repositorio está disponible públicamente con el código completo y esta guía para su implementación.
\end{abstract}
...
décadas. El cuerpo resultante es rico en sustancia pero opaco en estructura. Las conexiones entre documentos son difíciles de captar y no es trivial
...
\textit{corpus}, identifica cuáles son los conceptos doctrinales más citados, qué documentos
...
jurisprudencial de este proyecto. Produce doctrina y constituye el precedente que el sistema debe
rastrear. El Track~B revela el estándar práctico de la FNE, identificando qué investiga, qué
metodologías aplica, y cuándo archiva. El Track~C funciona como ancla normativa. Todo
...
RAG pueda citar cada considerandode forma unificada, se necesita un patrón de
...
\noindent
Los tres primeros lugares los ocupan conceptos doctrinales, lo que confirma 
...
\textit{pipeline} documentado. Los extractos se abreviaron; las
...
\begin{codebox}
/graphify query "cuáles son los nodos más conectados del grafo?"
\end{codebox}
...
\begin{codebox}
./rag/ask.sh "\textquestiondown Cuánto fue la multa impuesta a CDF? \textquestiondown La confirmó la CS?"
\end{codebox}
...
lo que transforma la materia prima en un mapa de doctrina. Sin ese diseño previo,
